...
import { ThumbsUp, ThumbsDown, Trash2, Plus, Search, Loader2, X, Save, ArrowRightLeft } from 'lucide-react';
...
function ConvoyView({ currentUser }) {
  ...
  // NYTT STATE: Håller koll på om konvojen är full i realtid
  ...
  const fetchProposals = async () => {
    ...
    if (!error) {
      setProposals(data || []);
      setIsConvoyFull((data || []).length >= 5); // Uppdatera status direkt
    }
    setLoading(false);
  };
  ...
  const triggerAddFlow = (draftData) => {
    ...
    // Vi kollar vår realtids-state som uppdaterades när pinnen sattes
    if (isConvoyFull) {
  ...
  const handleMapClick = async (latlng) => {
    ...
    // --- NYTT: Blixtsnabb koll i bakgrunden PRECIS när pinnen sätts ---
    supabase.from('proposals').select('*', { count: 'exact', head: true }).then(({ count }) => {
      if (count !== null) {
        setIsConvoyFull(count >= 5);
        // Om databasen skiljer sig från vad vi ser på skärmen, hämta hem den nya listan i smyg!
        if (count !== proposals.length) fetchProposals(); 
      }
    });

    // Hämtar gatuadress (tar ca 0.5 sekunder, vilket ger Supabase tid att svara ovan)
    try {
  ...
  // --- STÅLDÖRREN: SÄKRAR UPP SKAPANDET ---
  const handleCreateNew = async () => {
    if (!modalDraft.name.trim() || isSaving) return;
    setIsSaving(true);

    try {
      // 1. En allra sista säkerhetskoll i fall två personer trycker Spara exakt samtidigt
      ...
      // 2. Om den hann bli full under tiden du skrev in namnet:
      if (count >= 5) {
        alert("Någon hann före! Gränsen på 5 tips är nu nådd. Byt till ersättningsläget.");
      ...
      // 3. Om vi passerat båda spärrarna -> SPARA
  ...
  return (
    <div style={{ padding: '10px 20px 100px 20px' }}>
        ...
        <MapContainer center={mapCenter} zoom={13} style={{ height: '100%', width: '100%' }} zoomControl={false}>
          ...
          {tempMarker && (
            <Marker position={tempMarker} icon={redIcon}>
              <Popup autoOpen>
                {/* Knappen anpassar sig direkt tack vare pre-fetch */}
      ...
      <div style={{ position: 'relative', marginBottom: '25px', display: 'flex', gap: '10px' }}>
        ...
        <button 
          onClick={() => {
            // Gör en realtids-koll även när man klickar på sök-plus-knappen
            supabase.from('proposals').select('*', { count: 'exact', head: true }).then(({ count }) => {
              setIsConvoyFull(count >= 5);
